...
const {
  ...
  createVirtualVenue,
  getAllVenuesForMap,
  evaluateLocationPing,
} = require('./populive-checkin-logic');
const {
  sendInteraction,
  trackProfileView,
  respondToPulse,
...
const {
  initiatePulsePurchase,
  initiatePurchase,
  initiateVenuePulseCreditsPurchase,
  handleStripeWebhook,
} = require('./populive-payments-logic');
const {
  sendMessage,
  getMessages,
...
const {
  ...
  updateVenueDrink,
  removeVenueDrink,
} = require('./populive-venue-insights');
const {
  joinSquad,
  ...
  setTableLock,
} = require('./populive-connector-engine');
const {
  getLocalRanking,
  getGlobalRanking,
...
} = require('./populive-ranking-queries');
const {
  createMission,
  getAllMissions,
  getMissionsNearUser,
  completeMission,
  getMissionPreview,
} = require('./populive-missions-logic');
...
// più sotto).
app.get(
  '/api/auth/is-professional-connector',
  requireAuthOnly,
  ah(async (req, res) => {
...
app.post(
  ...
  ah(async (req, res) => {
    ...
    const result = await trackProfileView(
      { viewerId: req.userId, viewedUserId, arenaSessionId, viaHistoricalBoard },
      deps
    );
    res.json(result);
  })
...
// ------------------------------------------------------------
app.get(
  '/api/users/:userId/pulse-balance',
  requireOnboarded,
  ah(async (req, res) => {
...
app.get(
  '/api/users/:userId/blocked-pairs',
  requireOnboarded,
  ah(async (req, res) => {
...
app.post(
  '/api/dashboard/products/:productId/price',
  requireArchitect,
  ah(async (req, res) => {
...
app.get(
  '/api/dashboard/search-by-hashtag',
  requireArchitect,
  ah(async (req, res) => {
...
app.get(
  '/api/dashboard/find-user-by-phone',
  requireArchitect,
  ah(async (req, res) => {
...
app.post(
  '/api/dashboard/users/:userId/instant-influencer',
  requireArchitect,
  ah(async (req, res) => {
...
app.post(
  '/api/dashboard/users/:userId/professional-connector',
  requireArchitect,
  ah(async (req, res) => {
...
app.post(
  '/api/dashboard/users/:userId/start-chat',
  requireArchitect,
  ah(async (req, res) => {
...
app.post(
  '/api/dashboard/venues/:venueId/spending-config',
  requireArchitect,
  ah(async (req, res) => {
    ...
    const result = await updateVenueSpendingConfig(
      { venueId: req.params.venueId, thresholdCents, bonusPoints },
      { db }
    );
    res.json(result);
  })
...
app.post(
  '/api/dashboard/venues/:venueId/ranking-threshold',
  requireArchitect,
  ah(async (req, res) => {
...
app.get(
  '/api/dashboard/venues/:venueId/drinks',
  requireArchitect,
  ah(async (req, res) => {
...
app.delete(
  '/api/dashboard/venues/:venueId/drinks/:drinkId',
  requireArchitect,
  ah(async (req, res) => {
...
app.post(
  '/api/dashboard/award-table-spending',
  requireArchitect,
  ah(async (req, res) => {
...
app.get(
  '/api/dashboard/venues/:venueId/full-settings',
  requireArchitect,
  ah(async (req, res) => {
...
app.post(
  '/api/dashboard/venues/:venueId/commission',
  requireArchitect,
  ah(async (req, res) => {
    ...
    await db.query(`UPDATE venues SET commission_venue_pct = $1 WHERE id = $2`, [
      commissionVenuePct,
      req.params.venueId,
    ]);
    res.json({ success: true });
...
app.post(
  '/api/dashboard/venues/:venueId/pulse-prices',
  requireArchitect,
  ah(async (req, res) => {
...
app.post(
  '/api/dashboard/venues/:venueId/arena-hours',
  requireArchitect,
  ah(async (req, res) => {
...
app.get(
  '/api/venues/:venueId/pulse-prices',
  ah(async (req, res) => {
...
app.post(
  '/api/venues/:venueId/pulse-credits/purchase',
  requireOnboarded,
  ah(async (req, res) => {
    const { quantity } = req.body;
    const result = await initiateVenuePulseCreditsPurchase(
      { userId: req.userId, venueId: req.params.venueId, quantity },
      { db }
    );
    res.json(result);
  })
...
app.post(
  '/api/table/claim-as-professional',
  requireOnboarded,
  ah(async (req, res) => {
...
// ------------------------------------------------------------
app.get(
  '/api/arenas/:arenaSessionId/ranking',
  ah(async (req, res) => {
    ...
    if (threshold.belowThreshold) {
      return res.json({
        success: true,
        belowThreshold: true,
        currentCount: threshold.currentCount,
        minRequired: threshold.minRequired,
        ranking: [],
      });
    }
...
app.get(
  '/api/users/:userId/ranking-summary',
  ah(async (req, res) => {
    const { arenaSessionId } = req.query;
    ...
    const summary = await getUserRankingSummary(
      { userId: req.params.userId, arenaSessionId, viewerId: valid ? viewerId : null },
      { db }
    );
...
app.get(
  '/api/dashboard/venue-report/:venueId',
  requireArchitect,
  ah(async (req, res) => {
...
app.post(
  ...
  ah(async (req, res) => {
    ...
    const result = await createVirtualVenue(
      { name, area, latitude, longitude, venueType, minUsersForLocalRanking },
      { db }
    );
    res.json(result);
  })
...
app.post(
  '/api/dashboard/feature-flags/:key',
  requireArchitect,
  ah(async (req, res) => {
...
app.get(
  '/api/venues/:venueId/historical-checkins',
  requireOnboarded,
  ah(async (req, res) => {
...
// ------------------------------------------------------------
app.get(
  '/api/arenas/:arenaSessionId/guess-candidates',
  ah(async (req, res) => {
...
app.get(
  '/api/users/:userId/interaction-history',
  requireOnboarded,
  ah(async (req, res) => {
...
app.get(
  '/api/users/:userId/pending-received-interactions',
  requireOnboarded,
  ah(async (req, res) => {
...
app.get(
  '/api/users/:userId/sent-interactions-history',
  requireOnboarded,
  ah(async (req, res) => {
...
app.get(
  '/api/users/:userId/unseen-like-center-count',
  requireOnboarded,
  ah(async (req, res) => {
...
app.post(
  '/api/users/:userId/mark-like-center-seen',
  requireOnboarded,
  ah(async (req, res) => {
...
app.get(
  '/api/users/:userId/unseen-notification-count',
  requireOnboarded,
  ah(async (req, res) => {
...
app.post(
  '/api/users/:userId/mark-notifications-seen',
  requireOnboarded,
  ah(async (req, res) => {
...
app.post(
  '/api/users/:userId/notifications/dismiss',
  requireOnboarded,
  ah(async (req, res) => {
...
app.post(
  '/api/users/:userId/notifications/clear-all',
  requireOnboarded,
  ah(async (req, res) => {
...
app.post(
  '/api/users/:userId/pulses/:pulseId/dismiss',
  requireOnboarded,
  ah(async (req, res) => {
    const result = await dismissPulseView({ userId: req.userId, pulseId: req.params.pulseId }, deps);
    res.json(result);
  })
);

app.post(
  '/api/users/:userId/pulses/clear-all',
  requireOnboarded,
  ah(async (req, res) => {
    const result = await clearAllPulseViews({ userId: req.userId }, deps);
    res.json(result);
  })
);
...
app.get(
  '/api/users/:userId/unread-chat-count',
  requireOnboarded,
  ah(async (req, res) => {
...
app.post(
  ...
  requireOnboarded,
  ah(async (req, res) => {
    const result = await blockUserFromChat({ conversationId: req.params.conversationId, blockerId: req.userId }, deps);
    res.json(result);
  })
);

app.post(
  '/api/chat/:conversationId/keep-preference',
  requireOnboarded,
  ah(async (req, res) => {
...
app.post(
  ...
  ah(async (req, res) => {
    const {
      showRankingOnProfile,
      sponsoredMissionsEnabled,
      appearsInHistoricalSearch,
      receivePulsesEnabled,
      contactFilter,
      ghostModeEnabled,
      hapticNotificationsEnabled,
    } = req.body;
...
// ------------------------------------------------------------
// eslint-disable-next-line no-unused-vars -- Express riconosce il gestore errori SOLO se ha 4 parametri
...
httpServer.listen(PORT, () => {
  console.info(`PopuLive API in ascolto sulla porta ${PORT}`);
  ...
  console.info('Motore a orari avviato.');
});
